...
func buildTarget() {
	if *flagWin64 {
		fmt.Printf("build win64 leveldb.cgo ...\n")
	} else {
		fmt.Printf("build win32 leveldb.cgo ...\n")
	}
	...
	if *flagWin64 {
		cmdBuildInstall = exec.Command(
			`devenv`, `LEVELDB.sln`, `/build`, `Release|x64`, `/project`, `INSTALL.vcxproj`,
		)
	} else {
		cmdBuildInstall = exec.Command(
			`devenv`, `LEVELDB.sln`, `/build`, `Release|Win32`, `/project`, `INSTALL.vcxproj`,
		)
	}
	cmdBuildInstall.Dir = buildDir

	// dlltool -dllname leveldb.dll --def leveldb.def --output-lib libleveldb.a
	var cmdDll2A *exec.Cmd
	if *flagWin64 {
		cmdDll2A = exec.Command(
			`dlltool`,
			`-dllname`, `leveldb-cgo-win64.dll`,
			`--def`, `leveldb-cgo-win64.def`,
			`--output-lib`, `libleveldb-cgo-win64.a`,
		)
	} else {
		cmdDll2A = exec.Command(
			`dlltool`,
			`-dllname`, `leveldb-cgo-win32.dll`,
			`--def`, `leveldb-cgo-win32.def`,
			`--output-lib`, `libleveldb-cgo-win32.a`,
		)
	}
	cmdDll2A.Dir = "."
...
func installDll(dir_or_env string) {
	if dir := parseDir(dir_or_env); dir != "" {
		...
		if *flagWin64 {
			cpFile(dir+"/leveldb-cgo-win64.dll", "leveldb-cgo-win64.dll")
		} else {
			cpFile(dir+"/leveldb-cgo-win32.dll", "leveldb-cgo-win32.dll")
		}
	}
}
