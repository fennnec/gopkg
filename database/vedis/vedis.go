...
static int fetch_callback(const void* pData, unsigned int iDataLen, void *pUserData) {
	extern int go_vedis_InvokeFetchCallback(void*, void*, int);
	return go_vedis_InvokeFetchCallback(pUserData, (void*)pData, iDataLen);
}

static int vedis_kv_fetch_invoke_callback(vedis *pStore, const void *pKey, int nKeyLen, void *pUserData) {
	return vedis_kv_fetch_callback(pStore, pKey, nKeyLen, &fetch_callback, pUserData);
}

static void get_last_error_from_log(vedis *pStore, char *pDest) {
	int iLen = 0;
...
var (
	ErrReadOnly       = errors.New("vedis: Read-only database")
	ErrBusy           = errors.New("vedis: Another thread or process has an exclusive lock on the database")
	ErrIO             = errors.New("vedis: Input/Output error")
	ErrNoMemory       = errors.New("vedis: Out of Memory")
	ErrAbort          = errors.New("vedis: Another thread has released the database handle")
	ErrNotImplemented = errors.New("vedis: The underlying KV storage engine does not implement the xReplace() method")
	ErrPermission     = errors.New("vedis: Permission error")
	ErrLimit          = errors.New("vedis: Journal file record limit reached")
	ErrNotFound       = errors.New("vedis: Nonexistent record")
	ErrUnknown        = errors.New("vedis: Unknown error")
)
...
//export go_vedis_InvokeFetchCallback
func go_vedis_InvokeFetchCallback(pFn, pData unsafe.Pointer, iLen C.int) C.int {
