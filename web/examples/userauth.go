...
import (
	...
	_ "github.com/chai2010/gopkg/database/sqlite3"
...
var manager = web.NewSessionManager(logger)
...
func getSession(ctx *web.Context, manager *web.SessionManager) *web.Session {
	...
	ctx.SetSecureCookie("SessionId", web.Id, int64(manager.GetTimeout()))
...
func main() {
	...
	manager.OnStart(func(session *web.Session) {
		logger.Printf("Start session(\"%s\")", web.Id)
	})
	manager.OnEnd(func(session *web.Session) {
		logger.Printf("End session(\"%s\")", web.Id)
	})
